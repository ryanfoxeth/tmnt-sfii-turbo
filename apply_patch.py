...
# v29 all-fighter voice release.
V29_SHA256 = '03e1060d04be410d93d7a87e6668b51e6189d99ba887c526404f2c113df9001a'
V29_PATCH = 'tmnt-sfii-turbo-v28-to-v29.bps'
V29_FULL_PATCH = 'tmnt-sfii-turbo-v29.bps'
V29_PATCH_SHA256 = '193a288865eaa3929b30b6ad5add44978e3651141d03fd6aa985f72c07ed4f2b'
V29_FULL_PATCH_SHA256 = '12696e6843d1b94039ead4e426a33d32d9ee293261fa8f3187cec898a6a1d7d3'
...
def _require_release_metadata():
    ...
    if not V29_PATCH_SHA256:
        raise BPSError('v29 incremental patch metadata is missing')
    PATCH_SHA256[V29_PATCH] = V29_PATCH_SHA256
    PATCH_SHA256[V29_FULL_PATCH] = V29_FULL_PATCH_SHA256
...
def choose_patch(input_bytes: bytes):
    ...
    if digest == V29_SHA256:
        raise BPSError("input is already the v29 release; no patch is needed")
    if digest == V28_SHA256:
        return input_bytes, PATCHES / V29_PATCH
    ...
    raise BPSError("input SHA-256 is not a supported original, v12 through v28 revision")


def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("input", type=Path, help="supported original, v12 through v28 ROM")
    parser.add_argument("--out", required=True, type=Path, help="new v29 output file")
    ...
    try:
        ...
        if patch_path.name not in (V18_PATCH, V19_PATCH, V20_PATCH, V21_PATCH, V22_PATCH, V23_PATCH, V24_PATCH, V25_PATCH, V26_PATCH, V27_PATCH, V28_PATCH, V29_PATCH):
            ...
            target, _ = apply_bps(target, upgrade)
        if patch_path.name not in (V19_PATCH, V20_PATCH, V21_PATCH, V22_PATCH, V23_PATCH, V24_PATCH, V25_PATCH, V26_PATCH, V27_PATCH, V28_PATCH, V29_PATCH):
            ...
            target, _ = apply_bps(target, upgrade)
        if patch_path.name not in (V20_PATCH, V21_PATCH, V22_PATCH, V23_PATCH, V24_PATCH, V25_PATCH, V26_PATCH, V27_PATCH, V28_PATCH, V29_PATCH) and sha256(target) != V19_SHA256:
            raise BPSError("patched ROM SHA-256 does not match the v19 release")
        if patch_path.name not in (V20_PATCH, V21_PATCH, V22_PATCH, V23_PATCH, V24_PATCH, V25_PATCH, V26_PATCH, V27_PATCH, V28_PATCH, V29_PATCH):
            ...
            target, _ = apply_bps(target, upgrade)
        if patch_path.name not in (V21_PATCH, V22_PATCH, V23_PATCH, V24_PATCH, V25_PATCH, V26_PATCH, V27_PATCH, V28_PATCH, V29_PATCH):
            ...
            target, _ = apply_bps(target, upgrade)
        if patch_path.name not in (V22_PATCH, V23_PATCH, V24_PATCH, V25_PATCH, V26_PATCH, V27_PATCH, V28_PATCH, V29_PATCH):
            ...
            target, _ = apply_bps(target, upgrade)
        if patch_path.name not in (V23_PATCH, V24_PATCH, V25_PATCH, V26_PATCH, V27_PATCH, V28_PATCH, V29_PATCH):
            ...
            target,_=apply_bps(target,upgrade)
        if patch_path.name not in (V24_PATCH, V25_PATCH, V26_PATCH, V27_PATCH, V28_PATCH, V29_PATCH):
            ...
            target, _ = apply_bps(target, upgrade)
        if patch_path.name not in (V25_PATCH, V26_PATCH, V27_PATCH, V28_PATCH, V29_PATCH):
            ...
            target, _ = apply_bps(target, upgrade)
        if patch_path.name not in (V26_PATCH, V27_PATCH, V28_PATCH, V29_PATCH):
            ...
            target, _ = apply_bps(target, upgrade)
        if patch_path.name not in (V27_PATCH, V28_PATCH, V29_PATCH):
        ...
        if patch_path.name not in (V28_PATCH, V29_PATCH):
        ...
        if patch_path.name != V29_PATCH:
            if sha256(target) != V28_SHA256: raise BPSError("patched ROM SHA-256 does not match v28")
            target, _ = apply_bps(target, (PATCHES / V29_PATCH).read_bytes())
        if sha256(target) != V29_SHA256: raise BPSError("patched ROM SHA-256 does not match v29")
        ...
        return 1
    print("Wrote {} (SHA-256: {})".format(args.out, V29_SHA256))
    return 0
